...
const CreatedNews = () => {
  ...
  const updated = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.put(
        `${base_url}/api/v1/update/${news_id}`,
        formData,
        {
  ...
  const editNews = async () => {
    const { data } = await axios.get(`${base_url}/api/v1/news/${news_id}`, {
      headers: { Authorization: `Bearer ${store.token}` },
    });
    setTitle(data?.news?.title);
    setDescription(data?.news?.description);
    setImg(data?.news?.image?.url);
    setOldImage(data?.news?.image?.url);
  };
  ...
  return (
    <div className="bg-white rounded-md p-4">
      ...
      <div className="">
        <form onSubmit={updated} className="flex flex-col gap-6">
